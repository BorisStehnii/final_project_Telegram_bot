...
def update_type_data(update: Update, context: CallbackContext):
    ...
    logging.info(f'user: обновил данные клиента')
...
def start(update: Update, context: CallbackContext):
    logging.info(f'user: {update.effective_user.id} вошел в чат')
...
def search(update: Update, context: CallbackContext):
    logging.info(f'user: выбрал режим поиска')
    context.bot.send_message(chat_id=update.effective_chat.id,
...
def add_client(update: Update, context: CallbackContext):
    logging.info(f'user: выбрал режим добавления клиента')
...
def write(update: Update, context: CallbackContext):
    ...
    logging.info(f'user: добавил нового клиента')
...
# проверка параметров клиента после правок
...
# невалидный ввод сообщения либо команды
def handler_msg_outside_func(update: Update, context: CallbackContext):
    text_msg = "Прости, я тебя не понимаю. Выбери один из режимов меню"
    context.bot.send_message(chat_id=update.effective_chat.id, text=f'{text_msg}',
                             reply_markup=menu_keyboard())
...
def index():
    ...
    start_cmd = CommandHandler('start', start, Filters.user(user_id=ADMIN['USER_ID']))

    conv_search = ConversationHandler(
        entry_points=[
            CommandHandler('search', search, Filters.user(user_id=ADMIN['USER_ID'])),
        ...
        fallbacks=[
            CommandHandler('cancel', menu, Filters.user(user_id=ADMIN['USER_ID'])),
            CallbackQueryHandler(menu, pattern='menu')
        ],
    )
    conv_add_client = ConversationHandler(
        entry_points=[
            CommandHandler('add_client', add_client, Filters.user(user_id=ADMIN['USER_ID'])),
        ...
        fallbacks=[
            CommandHandler('cancel', menu, Filters.user(user_id=ADMIN['USER_NAME'])),
            CallbackQueryHandler(menu, pattern='menu'),
    ...
    dispatcher.add_handler(MessageHandler(Filters.text, handler_msg_outside_func, pass_user_data=True))
